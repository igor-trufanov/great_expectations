...
class DataProfilerProfileNumericColumnsDiffGreaterThanThreshold(DataProfilerProfileMetricProvider):
        ...
        for col, stats in limit_check_report_keys_copy.items():
            ...
            for stat, threshold in stats.items():
                ...
                if diff_val == "unchanged":  # In the case there is no delta
                    diff_val = 0
                is_greater = is_value_greater_than_threshold(diff_val, threshold)
                if not is_greater:
...
class ExpectProfileNumericColumnsDiffGreaterThanThreshold(ProfileNumericColumnsDiffExpectation):
...
if __name__ == "__main__":
    diagnostics_report = ExpectProfileNumericColumnsDiffGreaterThanThreshold().run_diagnostics()
